...
<head>
    ...
    <!-- Fonts: Google Fonts with Chinese font families -->
    <!-- Always include at least one Chinese font (Noto Sans SC / Noto Serif SC / LXGW WenKai) -->
...
<button class="edit-toggle" id="editToggle" title="Edit mode (E)">
    <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M12.1 1.3a1.5 1.5 0 0 1 2.1 2.1L5.6 12 2 14l2-3.6 8.1-9.1z"/></svg>
</button>
```
...
When the presentation includes financial data (charts, KPIs, tables), add these utilities:

### Chinese Number Formatting (Required for Financial Slides)

```javascript
/* Include in <script> when any slide contains financial data */
...
### ECharts Integration (Optional)

```html
<!-- Add before </body> when charts are needed -->
<script src="https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js"></script>
```

Chart containers must init on slide visibility. See [financial-charts.md](financial-charts.md) for complete configuration reference.

### Trend Indicators (CSS Only, No Emoji)

```css
/* Up/down arrows as CSS pseudo-elements — never use emoji */
.trend-up::before {
    content: '';
    ...
    border-bottom: 6px solid var(--color-positive, #dc2626);
    margin-right: 0.3em;
...
.trend-down::before {
    ...
    border-left: 4px solid transparent;
    border-right: 4px solid transparent;
    border-top: 6px solid var(--color-negative, #16a34a);
    margin-right: 0.3em;
    vertical-align: middle;
}
.trend-flat::before {
...
- `lang="zh-CN"` on `<html>` for Chinese presentations
